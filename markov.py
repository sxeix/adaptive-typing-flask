                ...
                if i != len(word)-1:
                    if self.__model.get(word[i]):
                        if self.__model[word[i]].get(word[i+1]):
                            self.__model[word[i]][word[i+1]]['count'] = int(self.__model[word[i]][word[i+1]]['count']) + 1
                        else:
...
def calculate_error(m1,m2):
  ...
  divided = np.divide(sub, m1.get_matrix(), out=np.zeros_like(sub), where=m1.get_matrix()!=0)
  result = np.multiply(divided, 100) # This occurs to create a percent out of 100 - might be easier to leave as decimal
    ...
    for item in range(0,25):
      ...
      if model.get(states[row]):
        if model.get(states[row]).get(states[item]):
          if int(model[states[row]][states[item]]['count']) > 3:
            by_error.append((states[row], states[item], error_list[row][item]))
